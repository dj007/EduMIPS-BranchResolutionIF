 ...
 *
 * @author Andrea Spadaccini, Simona Ullo, Antonella Scandura, Massimo Trubia
 * (FPU modifications)
 */
public class CPU {
    ...
    public static enum FPExceptions {

        INVALID_OPERATION, DIVIDE_BY_ZERO, UNDERFLOW, OVERFLOW
    };

    public static enum FPRoundingMode {

        TO_NEAREST, TOWARD_ZERO, TOWARDS_PLUS_INFINITY, TOWARDS_MINUS_INFINITY
    };
    ...
    /**
     * CPU status. READY - the CPU has been initialized but the symbol table
     * hasn't been already filled by the Parser. This means that you can't call
     * the step() method, or you'll get a StoppedCPUException.
     *
     * RUNNING - the CPU is executing a program, you can call the step() method,
     * and the CPU will fetch additional instructions from the symbol table
     *
     * STOPPING - the HALT instruction has entered in the pipeline. This means
     * that no additional instructions must be fetched but the instructions that
     * are already in the pipeline must be executed. THe step() method can be
     * called, but won't fetch any other instruction
     *
...
//GETTING PROPERTIES -----------------------------------------------------------------
    ...
    /**
     * Returns true if the specified functional unit is filled by an
     * instruction, false when the contrary happens. No controls are carried out
     * on the legality of parameters, for mistaken parameters false is returned
     *
     * @param funcUnit The functional unit to check. Legal values are "ADDER",
     * "MULTIPLIER", "DIVIDER"
    ...
    /**
     * Returns true if the pipeline is empty. In this case, if CPU is in
     * stopping state we can halt the pipeline. The sufficient condition in
     * order to return true is that fpPipe doesn't work and it hadn't issued any
     * instrution now in the MEM stage
     */
    ...
    /**
     * Returns the instruction of the specified functional unit , null if it is
     * empty. No controls are carried out on the legality of parameters, for
     * mistaken parameters null is returned
     *
     * @param funcUnit The functional unit to check. Legal values are "ADDER",
     * "MULTIPLIER", "DIVIDER"
     * @param stage The integer that refers to the stage of the functional unit.
    ...
    /**
     * Returns the number of Structural Stalls (Divider not available) that
    ...
    /**
     * Returns the number of Structural Stalls (Memory not available) that
     * happened inside the pipeline
     *
     * @return an integer
     */
    ...
    /**
     * Returns the number of Structural Stalls (EX not available) that happened
     * inside the pipeline
     *
     * @return an integer
     */
    ...
    /**
     * Returns the number of Structural Stalls (FP Adder and FP Multiplier not
     * available) that happened inside the pipeline
     *
     * @return an integer
     */
            ...
            if (fpPipe.getInstruction(SIMUL_MODE_ENABLED) == null) {
                try {
                    ...
                    if (masked) {
                        ...
                        } else // We must complete this cycle, but we must notify the user.
    ...
    /**
     * This method resets the CPU components (GPRs, memory,statistics, PC,
     * pipeline and Symbol table). It resets also the Dinero Tracefile object
    ...
    /**
     * Test method that returns a string containing the values of every FPR.
     *
     * @return a string
     */
